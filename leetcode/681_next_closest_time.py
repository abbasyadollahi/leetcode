...
class Solution:
    def latestTime(self, time: str) -> str:
        ...
        mmm = (
            max([num for num in permutations if num < mm])
            if mm != "00"
            else max([num for num in permutations if num < "60"])
        )
        hhh = hh
        if mmm >= mm:
            hhh = (
                max([num for num in permutations if num < hh])
                if hh != "00"
                else max([num for num in permutations if num < "24"])
            )
