...
class Solution:
    def gcdOfStrings(self, str1: str, str2: str) -> str:
        ...
        for i in range(len(short_string), 0, -1):
            ...
            if (
                len(short_string) % len(divisor) == 0
                and len(long_string) % len(divisor) == 0
                and short_string == divisor * (len(short_string) // len(divisor))
                and long_string == divisor * (len(long_string) // len(divisor))
            ):
                return divisor
