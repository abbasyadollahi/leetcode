...
class TimeMap:
    ...
    def get(self, key: str, timestamp: int) -> str:
        if key in self.store and timestamp in self.store[key]["keys"]:
            ...
            return self.store[key]["keys"].get(
                self._find_closest_timestamp(timestamp, self.store[key]["timestamps"]),
                "",
            )
        else:
            return ""

    def _find_closest_timestamp(self, timestamp: int, timestamps: list[int]) -> int | None:
