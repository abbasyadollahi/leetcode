...
class Test(unittest.TestCase):
    def test_works_when_no_users_are_active(self) -> None:
        self.assertEqual(monthly_charge("2018-10", plan, users), 0)

    def test_works_when_the_active_users_are_active_the_entire_month(self) -> None:
        expected_bill = 0
        expected_bill += 2 * 5_000
        self.assertAlmostEqual(monthly_charge("2020-12", plan, users), expected_bill, delta=1)

    def test_works_when_the_active_users_are_active_partial_month(self) -> None:
        expected_bill = 0
        expected_bill += 2 * 5_000
    ...
    def test_works_when_the_active_users_are_active_one_day(self) -> None:
        expected_bill = 0
